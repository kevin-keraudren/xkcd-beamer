...
# https://github.com/JohannesBuchner/matplotlib-xkcdify
...
def XKCDify(ax, mag=1.0,
    ...
    if ticks:
        for x,xtext in zip(ax.get_xticks(), ax.get_xticklabels()):
            ax.text(x, xaxis_loc - 0.08 * yspan * (2 * xticks_inside - 1)-4, xtext.get_text(),
                fontsize=10, ha='center', va='bottom' if xticks_inside else 'top', rotation=0)
        for y,ytext in zip(ax.get_yticks(), ax.get_yticklabels()):
            print y,ytext
            ax.text(yaxis_loc + 0.02 * xspan * (2 * yticks_inside - 1), y+23, ytext.get_text(),
    ...
    #prop = fm.FontProperties(fname='Augie.ttf', size=18)
    ...
    if leg is not None:
        ...
        for child in leg.get_children():
            if isinstance(child, pl.Line2D):
                ...
                x[1] += 1500
                print "LEGEND", x,y
                # new_x = np.linspace(x[0],x[1], 80)
                # new_y = np.interp(new_x,x, y)
                # x,y=new_x,new_y
                
                child.set_data(xkcd_line(x, y, mag=0.1, f1=15, f2=0.001,f3=50))
    ...
    ax.text(0.5 * (xax_lim[1] + xax_lim[0]), yax_lim[1],
            ax.get_title(),
            ha='center', va='bottom', fontproperties=prop, fontsize=30)
    ax.set_title('')
